...
class MesosBaseAgentCheck(object):
    ...
    def _load_metric(self, check_name, metric, tags = []):
        ...
        if hasattr(self, metric_type):
            ...
            self.log.warn( "Unable to send %s, metric of type %s not implemented by AgentCheck!" % (metric_name, metric_type) )
...
class BaseMesosCheck(object):
    ...
    def extract_tags(self, d):
        return [ "%s:%s" % (k, d[k] if k in d else None) for k in self.metric_tag_keys ]
    ...
    def run(self):
        ...
        self._app_metric('gauge')('response_time', timing, tags=['url:%s:%s' % ('http_check',self.url)])
    ...
    def timeout_event(self):
        self._event_template(
                title = "url timeout",
                text  = "[%s] timed out after [%s] seconds." % (self.url, self.timeout)
        )

    def connection_error_event(self):
        self._event_template(
                title = 'connection error',
                text  = 'unable to connect to [%s].' % (self.url)
        )

    def empty_json_error(self):
        self._event_template(
                title = 'returned an empty json',
                text  = 'the json received from url [ %s ] has no content.' % (self.url)
        )

    def status_code_event(self, r):
        self._event_template(
                title = 'invalid reponse code',
                text  = '[ %s ] returned a status of [ %s ]' % (self.url, r.status)
        )
...
class MesosMasterStateCheck(BaseMesosCheck):
    ...
    def __init__(self, base_url, timeout, tags = []):
        super(MesosMasterStateCheck, self).__init__(
                base_url, timeout, self._name, tags, self._stat_url_suffix)
